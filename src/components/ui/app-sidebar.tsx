...
export function AppSidebar() {
  return (
    <Sidebar className="flex flex-col h-screen w-64 bg-background border-r">
      {/* SidebarContent tüm alanı kaplıyor */}
      <SidebarContent className="flex flex-col h-full">
        {/* Üst Header: Logo */}
        <div className="flex items-center justify-center h-16 border-b border-gray-300">
          {/* Logo için SVG – boyutunu istediğiniz gibi ayarlayın */}
          <svg
            id="logo-70"
            width="120"
            height="80"
        ...
        <SidebarGroup className="flex-1">
          {/* Ayraç altındaki içerik: Menü Öğeleri */}
          <SidebarGroupContent>
            <SidebarMenu>
              {items.map((item) => (
                <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild>
                    <a
                      href={item.url}
                      className="flex items-center space-x-3 px-4 py-2 text-sm font-medium text-muted-foreground hover:bg-muted/20 rounded transition-colors"
                    >
        ...
        {/* Alt Bölüm: Çıkış Yap Butonu */}
        <div className="px-4 py-4 border-t border-gray-300">
          <SidebarMenuButton asChild>
            <a
              href="#"
              className="flex items-center space-x-3 px-4 py-2 text-sm font-medium text-destructive hover:bg-destructive/10 rounded transition-colors"
            >
